...
import { enUS, Locale, itCH as itIT } from 'date-fns/locale';
import { BehaviorSubject } from 'rxjs';
import { AppSettings, AppTheme, defaults } from '../settings';
import { LocalStorageSettingService } from '@shared/services/storagesettings.service';

@Injectable({
  providedIn: 'root',
})
export class SettingsService {
  private readonly key = 'dym-settings';

  private readonly document = inject(DOCUMENT);
  private readonly translate = inject(TranslateService);
  private readonly store = inject(LocalStorageSettingService);
  ...
  localeMap: Record<string, Locale> = { 'en-US': enUS, 'it-IT': itIT };
