...
(function(root, factory) {
            ...
            return (function parse(data) {
                ...
                data.rules.forEach(function(rule) {
                    ...
                    function make_query(rule) {
                        ...
                        if (ope.nb_inputs !== 0) {
                            var es_key_val = {};
                            var queryDSLWord = getQueryDSLWord(rule, that);
                            var field = rule.field;

                            if (queryDSLWord === 'term' && that.getFilterById(rule.id).type === 'string' && field !== '_index') {
                                field += '.keyword';
                            }
                            es_key_val[field] = mdb.call(that, get_value(rule));
                            part[queryDSLWord] = es_key_val;
                        }
    ...
    */
    function getQueryDSLWord(rule, self) {
        var filter = self.getFilterById(rule.id);
