...
directiveModule.directive('ngDropdownMultiselect', ['$filter', '$document', '$compile', '$parse', function($filter, $document, $compile, $parse) {
	return {
		...
		template: function(element, attrs) {
			...
			var template = '<div class="multiselect-parent dropdown-multiselect" ng-class="{open: open}">';
			...
			template += '<ul class="dropdown-menu-custom dropdown-menu-form" ng-if="open" ng-style="{display: open ? \'block\' : \'none\', height : settings.scrollable ? settings.scrollableHeight : \'auto\', overflow: \'auto\' }" >';
        ...
        if ($scope.open) {
          $scope.close()
			...
			$scope.settings = {
				...
				closeOnSelect: false,
				buttonClasses: 'dropdown-custom col-12 span12 ',
			...
			$scope.texts = {
				...
				buttonDefaultText: 'Select Indexes',
