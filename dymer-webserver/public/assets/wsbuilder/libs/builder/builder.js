...
/* iframe */
	_loadIframe : function(url) {	
        ...
        var htmlHead =
                ...
                '\n <style>.collectionField {  border: 1px solid #ced4da;  border-top-left-radius: .25rem; border-bottom-left-radius: .25rem; padding: 8px;  border-left-color: rgb(70 82 126); border-left-width: 4px;}</style>';
...
/* iframe highlight */    
	...
	_initBox: function() {
		...
		/*VL document.getElementById("save-reusable-btn").addEventListener("click", function(event) {
...
/* drag and drop */
	...
	getHtml: function(keepHelperAttributes = true) {
		console.log(">>>>>getHtml ");
		let doc = window.FrameDocument;//OK
		let hasDoctpe = (doc.doctype !== null);//OK
		let html = "";//OK
		console.log(">>>hasDoctpe ", hasDoctpe);
	...
	saveElement: function(element, type, name, callback) {
		...
		.then((data) => {
			...
			//TODO check displayToast(bg, "Save", data.message ?? data);					
		})
		.catch(error => {
			console.log(error.statusText);
			//TODO check displayToast("bg-danger", "Error", "Error saving!");
		});
		/*
		return $.ajax({
			type: "POST",
	...
	saveAjax: function(data, saveUrl, callback, error ) {
		console.log(">>>>>>saveAjax drag and drop ");
	...
	saveAjax: function(action, infodata, callback) {
        ...
        switch (action) {
            case "create":
                ...
                pageId = null;
                /*VL*/
				var s = infodata.file.src;//FIX VL 31 ott
				if (Vvveb.pathservice.includes("api/forms")) {
					var html3_bis = "";
					let code = s.toString();
					$(code).find("[dymer-model-element]").each(function() {
						html3_bis += this.outerHTML;
					});
					var json_struct_bis = JSON.stringify((html2json(html3_bis)));
					infodata.structure =  JSON.parse(json_struct_bis);
				}
				/*VL*/
...
Vvveb.CodeEditor = {
	...
	init: function(doc) {
		...
		console.log(">>>>init ", Vvveb.Builder.getHtml());
	...
	setValue: function(value) {
		if (this.isActive) {
			console.log(">>>>>setValue ", Vvveb.Builder.getHtml());
			this.textarea.value = Vvveb.Builder.getHtml();
		}
	},
...
//TODO check eliminato temporaneamente
...
Vvveb.Gui = {
   ...
   let components_=$("#components").find("[data-vvveb-action='collapse']"); 
   Vvveb.Gui.autocollapse();
	},
	...
	save : function () {
		console.log(">>>>save show modal with html content ", Vvveb.Builder.getHtml());
	...
	saveAjax : function () {
		console.log(">>>>>>saveAjax Vvveb.Gui ");
		let btn = this;
	...
	//FS	btn.querySelector(".loading").classList.remove("d-none");
	//FS	btn.querySelector(".button-text").classList.add("d-none");
        ...
        return Vvveb.Builder.saveAjax("update", undefined, function(data) {
			console.log(">>>>old dymer save in Vvveb.Gui");
            var pageId = Vvveb.FileManager.getCurrentPage();
	...
	downloadNEW : function () {
		console.log(">>>>>>download Vvveb.Gui ");
	...
	downloadold: function() {
		console.log(">>>>TODO downloadold zip lib");
        ...
        console.log(">>>>TODO checl zip lib, zipName ", zipName);
		...
		console.log(">>>>TODO check zip lib, content_readme ", content_readme);
	...
	viewport : function () {
		console.log(">>>>>>viewport Vvveb.Gui ");
	...
	toggleEditor : function () {
		console.log(">>>>>>toggleEditor Vvveb.Gui ");
	...
	fullscreen : function () {
		console.log(">>>>>>fullscreen Vvveb.Gui ");
   ...
   autocollapse : function () {
       var e = document.querySelectorAll('input.header_check[type="checkbox"]').forEach(e => e.checked = false);
	},
	//Pages, file/components tree 
	/*newPage : function () {
		console.log(">>>>>>newPage Vvveb.Gui ");
	...
	//old dymer code
 	...
 	newMagic: function(){
		...
		var newMagicModal = $('#new-magic-modal');

		newMagicModal.modal("show").find("form").off("submit").submit(function(event) {


			event.preventDefault();
			var startTemplateUrl = "";

			var entitytype = $("[name=entitytype]", newMagicModal).val();
			entitytype = (entitytype.replace(/[^a-zA-Z]/g, "")).toLowerCase();

			var name_list = entitytype+"_list";

			var name_full = entitytype+"_full";
  	...
  	newPage: function(){
        ...
        newPageModal.modal("show").find("form").off("submit").submit(function(event) {
            ...
            startTemplateUrl = "";
                ...
                if (isValidJsonString(html_string)){
                    let JSONstartTemplateUrl = JSON.parse(html_string);
                    startTemplateUrl = JSONstartTemplateUrl["data"];
                } else {
                ...
                //startTemplateUrl = JSON.parse(html_string);
                    ...
                    data.forEach(element => {
                        ...
                        element.files.forEach(elfiles => {
                            if (elfiles.contentType == "text/html") {
                                /*MG - Aggiunto nome del modello nell'url*/
                                if (Vvveb.editorType == "templates") {
                                    singlePage.url = Vvveb.pathservice + "content/" + elfiles._id
                                }else{    
                                    singlePage.url = Vvveb.pathservice + "content/" + elfiles.title + "/" + elfiles._id
                                }
                        ...
                        /*VL Vvveb.FileManager.scrollBottom();*/
                    });
                });
            }, 'html');
...
Vvveb.FileManager = {
	...
	init: function(allowedComponents = {}) {
		...
		//start old dymer event
		this.tree.addEventListener("click", function (e) {
			let element = event.target.closest(".editpage");
			if (element) {
				Vvveb.FileManager.editPage(element.closest("li"), e, false);
				e.stopImmediatePropagation();
				e.preventDefault();
				return false;
			}
		...
		this.tree.addEventListener("click", function (e) {
			let element = event.target.closest(".editassetpage");
			if (element) {
				Vvveb.FileManager.editAssetsPage(element.closest("li"), e, false);
				e.stopImmediatePropagation();
				e.preventDefault();
				return false;
			}
	...
	/*
	deletePageNEW: function(element, e) {
		console.log(">>>>>deletePage Vvveb.FileManager");
...
*/
	...
	deletePage: function(element, e) {
		console.log(">>>>>old dymer deletePage Vvveb.FileManager");
		let page = element.dataset;
		let modelId = page.page;
		var dataedit = this.pages[modelId];		
		var datapost = { pageId: modelId };
		...
		deletePageModal.modal("show").find("form").off("submit").submit(function(event) {
			return Vvveb.Builder.saveAjax("delete", datapost, function(data) {
				...
				if (data.success) {
					delete Vvveb.FileManager.pages[modelId];
					$('#filemanager .tree li[data-page="' + modelId + '"]').remove();
					$('#message-modal').modal().find(".modal-body").html("Delete successfully! ");
					if (modelId == Vvveb.FileManager.getCurrentPage())
 ...
 /*FS*/
	...
	//end old dymer
	//start old dymer function
	//TODO test template case
	editPage: function(element, e) {
		console.log(">>>>>old dymer editPage Vvveb.FileManager");
		let page = element.dataset;
		let modelId = page.page;
	...
	//start old dymer (blocks of js and css)
	addAsset: function(el) {
        var editPageModal = $(el).closest('form');
        var datapost = {};
    ...
    editAssetsPage: function(element, e) {
		console.log(">>>>>old dymer editAssetsPage Vvveb.FileManager");
		let page = element.dataset;
		let modelId = page.page;       		
		name = modelId;
	...
	renamePage: function(element, e, duplicate = false) {
		console.log(">>>>>renamePage Vvveb.FileManager");
		let page = element.dataset;
		...
		if (newfile) {
			...
			if (page) {
				...
				.then((data) => {
						...
						//TODO check displayToast(bg, "Rename", data.message ?? data);
				...
				.catch(error => {
					...
					//TODO check displayToast("bg-danger", "Error", message);
	...
	addPage: function(name, data, afterPage = false) {
		console.log(">>>>>addPage Vvveb.FileManager");
	...
	addPages: function(pages) {
		for (page in pages) {
			//console.log(">>>pages[page][name]: ", pages[page]['name']);
